    ...
    try (
      ...
      while (true) {
        ...
        ReceiveMessageRequest request = ReceiveMessageRequest.builder().queueUrl(queueURL)
            .maxNumberOfMessages(10).waitTimeSeconds(20).messageAttributeNames("Body")
            .build();
        ReceiveMessageResponse response = sqsClient.receiveMessage(request);

        for (Message message : response.messages()) {
          AddressBook ab = AddressBook.parseFrom(
              message.messageAttributes().get("Body").binaryValue().asByteArray());
